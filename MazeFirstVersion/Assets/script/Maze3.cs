...
public class Maze3 : MonoBehaviour {
	...
	public void ChangeLayer(IntVector2 coordinates){
		IntVector2 coor1 = new IntVector2 (coordinates.x - 2, coordinates.z - 2);
		IntVector2 coor2 = new IntVector2 (coordinates.x + 2, coordinates.z + 2);
		...
		for(int x = coor1.x; x <= coor2.x; x++){
			for(int z = coor1.z; z <= coor2.z; z++){
				if ((x - coordinates.x) * (x - coordinates.x) + (z - coordinates.z) * (z - coordinates.z) <= 4) {
					...
					for (int num = 0; num < mazeWallNum [x, z]; num++) {
						...
						if (objectGrandChild.layer == 14) {
							objectGrandChild.layer = LayerMask.NameToLayer ("viewWall");
							lists.Add (objectGrandChild);
						}
					}
				}
			}
	...
	public IEnumerator UnViewList(){
		WaitForSeconds delay = new WaitForSeconds (unviewDelay);
		while (true) {
			yield return delay;
			if (lists [0].layer == 13) {
				lists[0].layer = LayerMask.NameToLayer ("unviewWall");
			}
			lists.RemoveAt (0);
		}
	}
